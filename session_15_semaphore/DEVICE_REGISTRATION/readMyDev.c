...
ssize_t readMyDev ( struct file *file_localp, char __user *ubuff,size_t size,loff_t *lofset)
{
	...
	if(down_interruptible(&localDev->sem))
	{
		...
		printk(KERN_INFO "%s: after  semaphore---------------- \n",__func__);
	...
	item = localDev->first;
		...
		while( local_size > 0 )
                {
			printk(KERN_INFO "%s: INSIDE while   -----  after  semaphore---------------- \n",__func__);
	...
	file_localp->f_pos = *lofset;
		...
		printk(KERN_INFO "%s: before releasing  semaphore---------------- \n",__func__);
	up(&localDev->sem);
		printk(KERN_INFO "%s: after releasing  semaphore---------------- \n",__func__);
