	...
	size_t local_size = 0;
		...
		item = localDev->first;
                while( local_size > 0 )
                {
                        ...
                        uc /*no_of_unsuccess_read*/ = copy_to_user( (ubuff+csr) , item->data[i] , ctr /*no_of_char_to_read*/);
                        if(uc /*no_of_unsuccess_read*/ > 0)
                        {
                                printk(KERN_INFO " PARTIAL_READ \n");
                        }

                        csr/*no_of_char_successfully_read*/ =csr /* no_of_char_successfully_read*/ + ( ctr /*no_of_char_to_read*/ - uc /*no_of_unsuccess_read*/ );
                        local_size = local_size - (ctr /*no_of_char_to_read*/ - uc /* no_of_unsuccess_read*/);
