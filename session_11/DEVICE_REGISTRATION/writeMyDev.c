...
ssize_t  writeMyDev (struct file *file_localp, const char __user *ubuff, size_t size, loff_t *lofset)
{
	...
	if(validIOCheck( file_localp, ubuff, size))
	{
		...
		while( local_size > 0 )
		{
			...
			uc /*no_of_unsuccess_char*/ = copy_from_user( item->data[i] , ubuff , ctw /*no_of_char_to_write*/);
			...
			csw/*no_of_char_successfully_wrote*/ =csw /* no_of_char_successfully_wrote*/ + ( ctw /*no_of_char_to_write*/ - uc /*no_of_unsuccess_char*/ );
