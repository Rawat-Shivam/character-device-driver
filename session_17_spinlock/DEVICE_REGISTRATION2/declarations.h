...
/*
 * this is the lock and unlock used 
 * above and below critical sections
	
 	spin_lock(&localDev->lock);
 	spin_unlock(&localDev->lock);

 here we didnt find any critical section aplicable for spinlock
 *
 *
 */
